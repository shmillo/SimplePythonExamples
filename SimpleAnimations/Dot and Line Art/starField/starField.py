...
class Star():
    ...
    def __init__(self):
        ...
        self.sx = 0.0; self.sy = 0.0; self.size = 0.0; self.pz = 0.0
    ...
    def showTrail(self):
        self.tx = normalizeScalar(self.sy/self.pz, 0.0, 1.0, 0.0, width)
        self.ty = normalizeScalar(self.sy/self.pz, 0.0, 1.0, 0.0, height)
        self.pz = self.z
...
def animationFunc(i):
    ...
    for i in range(len(stars)):
        ...
        trailArray[0][i], trailArray[1][i], trailArray[2][i], trailArray[3][i] = stars[i].showTrail()
        stars[i].update()
...
trailArray = np.zeros([4, len(stars)])
