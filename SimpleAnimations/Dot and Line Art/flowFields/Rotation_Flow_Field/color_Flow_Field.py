...
def animFunc(i):
    ...
    for j in range(angleOfRotation.shape[0]):
        for k in range(angleOfRotation.shape[1]):
            #ax.fill_between(X[j][k], Y[j][k], c=yRotation[timeStep][j][k])
            ax.arrow(X[j][k], Y[j][k], xRotation[timeStep][j][k]*0.1, yRotation[timeStep][j][k]*0.1, color=mplt.cm.prism(yRotation[timeStep][j][k]), head_width=0.0)
    frameIndex += 1
...
totalNumFrames = 200; maxPoints = 5.0; step = 0.1; size = 0.8; stepBySize = step*size; frameIndex = 0
xPoints = np.arange(0, maxPoints, step); yPoints = np.arange(0, maxPoints, step); X, Y = np.meshgrid(xPoints, yPoints)

angleOfRotation = np.zeros_like(X); rate = np.zeros_like(X); rateInc = 10.339; time = 0.0; numTimeSteps = totalNumFrames
for i in range(rate.shape[0]):
    for y in range(rate.shape[1]):
        rate[i][y] = rateInc; rate[y][i] = rateInc
...
xRotation *= (invNorm); yRotation *= (invNorm)
xRotation = abs(xRotation); yRotation = abs(yRotation)
print(xRotation)
#animFunc(40)

anim = FuncAnimation(fig, func=animFunc, frames=np.linspace(0, totalNumFrames, num=totalNumFrames), interval=1)
anim.save("color_Flow_Field_RYG_01.gif", fps=10); print('done')
mplt.show()