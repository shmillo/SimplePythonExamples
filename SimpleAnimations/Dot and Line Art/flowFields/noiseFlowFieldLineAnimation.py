...
def animFunc(i):
    global X, Y, valueNoise, angleOfRotation, maxPoints, step, size, stepBySize, xPoints, yPoints, niX, niY

    ax.clear(); ax.axis('off')
    for j in range(valueNoise.shape[0]):
        for k in range(valueNoise.shape[1]):
            valueNoise[j][k] = Noise(niX, 0.6, 0.51); 
            niX += 0.0000091 + ( 0.000011 * np.random.rand() )

    angleOfRotation = valueNoise*np.pi*2.0; 
    ...
    for j in range(valueNoise.shape[0]):
        for k in range(valueNoise.shape[1]):
            ax.arrow(X[j][k], Y[j][k], stepBySize*xRotation[j][k], stepBySize*yRotation[j][k], head_width=0.0)
...
anim = FuncAnimation(fig, func=animFunc, frames=np.linspace(0, 100, num=100), interval=1000)
anim.save("flowfield_Line_Animation_Perlind_2D.gif", fps=20)
mplt.show()