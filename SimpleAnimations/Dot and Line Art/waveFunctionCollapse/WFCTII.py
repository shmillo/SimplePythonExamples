...
class Tile( object ):
    ...
    def rotate(self, num):
        newImage = np.rot90(self.image, k=num, axes=(1,0))
        newEdges = []
        lenEdges = len(self.edges)
        for i in range(lenEdges):
            newEdges.append(self.edges[((lenEdges - num) + i)%lenEdges])
...
tileImages = []
tileImages.append(np.asarray(Image.open("/Users/shawnmilloway/Desktop/wfcPhotos/selfMadeWfcPhotos/GroundTiles/forestOne/1-01.png").convert("RGBA")))
tileImages.append(np.asarray(Image.open("/Users/shawnmilloway/Desktop/wfcPhotos/selfMadeWfcPhotos/GroundTiles/forestOne/2-01.png").convert("RGBA")))
tileImages.append(np.asarray(Image.open("/Users/shawnmilloway/Desktop/wfcPhotos/selfMadeWfcPhotos/GroundTiles/forestOne/3-01.png").convert("RGBA")))
tileImages = np.array(tileImages)

#AAA BBB ACA ABA 
tiles = []
tiles.append( Tile(tileImages[0], ["AAA", "AAA", "AAA", "AAA"]) )
tiles.append( Tile(tileImages[1], ["AAA", "ABA", "AAA", "ABA"]) )
tiles.append( Tile(tileImages[2], ["ABA", "ABA", "AAA", "AAA"]) )
for i in range(4):
    for j in [1,2]:
        tiles.append(tiles[j].rotate(i))
for tile in tiles:
    tile.analyze(tiles)
