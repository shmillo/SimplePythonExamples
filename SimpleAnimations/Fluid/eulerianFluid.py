...
def animFunc(i):
    global X, Y, forceLocationX, forceLocationY, dt, gravity, p, v, s, u, m, vx, vy, dx, dy, time
    ...
    for j in range(len(X)):
        ax.scatter(X[j], Y[j], m[0][j], c=pressureColor[j]/255.0, cmap='prism')
    #ax.contourf(X, Y, m[0])

overRelaxation = 1.9; dt = 1.0/60.0; density = 2000.0; nX = 300; nY = 300; 
res = 50.0; domainHeight = 1.0; domainWidth = domainHeight / nX*nY; h = domainHeight / res
cp = density*h/dt; gravity = -9.81; time = 0.0
...
obstacleRadius = 0.030; obstacleRadiusSquared = obstacleRadius**2.0
forceLocationX = 0.45; forceLocationY = 0.45
